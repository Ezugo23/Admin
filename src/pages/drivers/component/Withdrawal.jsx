...
const Withdrawal = ({ driverId }) => {
  ...
  const { withdrawals, loading, error, fetchTransactions, setCurrentDriverId } =
    usePayoutContext();
  ...
  const { currentWithdrawals, totalPages } = useMemo(() => {
    const indexOfLastItem = currentPage * itemsPerPage;
    const indexOfFirstItem = indexOfLastItem - itemsPerPage;
    return {
      currentWithdrawals: withdrawals.slice(indexOfFirstItem, indexOfLastItem),
      totalPages: Math.ceil(withdrawals.length / itemsPerPage),
    };
  }, [currentPage, itemsPerPage, withdrawals]);
  ...
  const renderPaginationButtons = () => {
    ...
    return pageNumbers.map((page, index) => (
      <Button
        ...
        variant={currentPage === page ? '#4CAF50' : 'outline'}
    ...
    return (
      <Center h={'400px'}>
        <VStack
          ...
          transition="all 0.7s"
        >
          <Text>No withdrawal history available for this driver</Text>
          <Button
      ...
      <Box bg={'white'} p={'5'}>
                ...
                >
                  ...
                  >
                    <Badge
                      variant="subtle"
                      colorScheme={
                        withdrawal.status === 'success' ? 'green' : 'red'
                      }
                      borderRadius="full"
                      px="2"
                      py="1"
                      fontSize="0.675rem"
                    >
                      {withdrawal.status === 'success' ? 'Success' : 'Pending'}
                    </Badge>
                  </Td>
                </Tr>
              ))}
        ...
        <Flex justify="space-between" align="center" mt={4}>
          <Text fontSize="0.875rem">
            ...
            {Math.min(currentPage * itemsPerPage, withdrawals.length)} of{' '}
