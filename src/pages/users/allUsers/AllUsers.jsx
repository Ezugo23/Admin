...
const AllUsers = () => {
  ...
  let content;
  if(isPending) {
    content = <div className='absolute left-[52%] top-[50%] -translate-x-[50%] text-black text-5xl'><i class="fa-solid fa-spinner fa-spin"></i></div>
  }
          ...
          {currentUsers.map((user, index) => (
            <tr key={user._id} className="table-row cursor-pointer">
              ...
              <td className="flex items-center gap-3">
                <Link to={`/users/editUser/${user._id}`} className="action-item cursor-pointer flex items-center gap-2" >
                  <FaEdit /> Edit
                </Link>
                <span onClick={() => handleSuspendClick(user._id, user.isActive)} className="action-item cursor-pointer flex items-center gap-1">
...
return (
  <>
    <div className="contain">
      ...
      {content}
      {openModal && <SuspendModal setOpenModal={setOpenModal} userID={userId} isActive={userStatus}/>}
    </div>
    </>
  );
};
